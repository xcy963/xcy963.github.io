...
import { copyFileSync, existsSync, rmSync, RmOptions } from "fs";
...
class Paster {
    ...
    switch (ctx_type) {
      case xclip.ClipboardType.Html:
        if (enableHtmlConverter) {
          ...
          if (text) {
            if (Paster.tryPasteFromFilePath(text)) {
              return;
            }
  ...
  private static tryPasteFromFilePath(text: string): boolean {
    const filePath = Paster.extractFilePath(text);
    if (!filePath) return false;
    const ext = path.extname(filePath).toLowerCase();
    if (!Paster.isImageExt(ext)) return false;
    Paster.pasteImageFromLocalPath(filePath);
    return true;
  }
  ...
  private static extractFilePath(text: string): string | null {
    ...
    let raw = lines[0].replace(/^"+|"+$/g, "").replace(/^'+|'+$/g, "");
  ...
  private static isImageExt(ext: string): boolean {
    return [".png", ".jpg", ".jpeg", ".gif", ".bmp", ".webp", ".svg"].includes(
      ext
    );
  }

  private static pasteImageFromLocalPath(sourcePath: string) {
    const ext = path.extname(sourcePath) || ".png";
    const imagePath = Paster.genTargetImagePath(ext);
    if (!imagePath) return;
    const silence = Paster.getConfig().silence;
    if (silence) {
      Paster.copyLocalImageAndRender(sourcePath, imagePath);
      return;
    }
    const options: vscode.InputBoxOptions = {
      prompt: "你可以改一改文件名字,我会帮你写到对应的文件夹里面放心就好",
      value: imagePath,
    ...
    vscode.window.showInputBox(options).then((inputVal) => {
      if (!inputVal) return;
      Paster.copyLocalImageAndRender(sourcePath, inputVal);
    });
  }

  private static copyLocalImageAndRender(
    sourcePath: string,
    targetPath: string
  ) {
    const pasteImgContext = Paster.parsePasteImageContext(targetPath);
    if (!pasteImgContext || !pasteImgContext.targetFile) return;
    const outPath = pasteImgContext.targetFile.fsPath;
    if (!prepareDirForFile(outPath)) {
      vscode.window.showErrorMessage("Make folder failed:" + outPath);
      return;
    }
    try {
      copyFileSync(sourcePath, outPath);
    } catch (err) {
      vscode.window.showErrorMessage("Copy image failed:" + err);
      return;
    }
