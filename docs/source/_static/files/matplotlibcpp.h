...
bool scatter(const std::vector<NumericX>& x,
  ...
  if (!fig) throw std::runtime_error("Call to figure() failed.");
    ...
    PyTuple_SetItem(args_subplot, 0, PyLong_FromLong(1));   // nrows = 1
    PyTuple_SetItem(args_subplot, 1, PyLong_FromLong(1));   // ncols = 1
    PyTuple_SetItem(args_subplot, 2, PyLong_FromLong(1));   // index = 1
    ...
    if (!axis) {
        PyErr_Print();
        throw std::runtime_error("No axis (add_subplot failed)");
    }
...
inline void xlabel(const std::string &str, const std::map<std::string, std::string> &keywords = {})
{
    ...
    for (auto it = keywords.begin(); it != keywords.end(); ++it) {
        ...
        // ✅ 检测是否为纯数字
        bool is_number = !value.empty() && std::find_if(value.begin(), value.end(), [](unsigned char c) {
            return !(std::isdigit(c) || c == '.' || c == '-');
        }) == value.end();

        if (is_number)
            PyDict_SetItemString(kwargs, key.c_str(), PyFloat_FromDouble(std::stod(value)));
        else
            PyDict_SetItemString(kwargs, key.c_str(), PyUnicode_FromString(value.c_str()));
    }
...
inline void zlabel(const std::string &str, const std::map<std::string, std::string>& keywords = {})
{
    detail::_interpreter::get();

    PyObject* axis = PyObject_CallObject(detail::_interpreter::get().s_python_function_gca, nullptr);
    if (!axis) throw std::runtime_error("Failed to get current axis (gca)");

    PyObject* set_zlabel = PyObject_GetAttrString(axis, "set_zlabel");
    if (!set_zlabel) throw std::runtime_error("No set_zlabel() on axis");
    ...
    PyObject* res = PyObject_Call(set_zlabel, args, kwargs);
    if (!res) {
        PyErr_Print();
