...
```{note}
也就是说makefile里面都是`目标文件:`,`前置文件`,`换行tab`,`相关的生成命令`
- 核心目标是生成文件
- 如果文件存在就会比较时间戳,(目标文件和他的前置文件的时间)
  - 如果前置文件的修改时间晚于目标文件,说明目标文件生成之后他的前置有发生更改,那么就需要重新执行生成的命令(**如果命令里面没有显示的删除操作,那么make不会自动删除**)
- 其实这里target可以不是文件,比如`.PHONY: all clean run`修饰的目标代表只是一种命令,这个可以在makefile顶部声明,被修饰的target就不会生成叫做all的文件
- 自动变量:
  - `$@`：当前规则的目标名
  - `$<`：当前规则的第一个依赖名
```
...
$(KERNEL_RV): $(OS_BIN)
	cp $< $@
#就是把$(OS_BIN)文件(这个是一个目录)复制成$(KERNEL_RV)
```
